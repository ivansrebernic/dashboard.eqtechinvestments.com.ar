...
serve(async (req) => {
  try {
    ...
    // Parse request
    const { portfolioId, forceRecalculation = false } = await req.json()

    console.log('Portfolio snapshot calculation started', {
    ...
    let portfoliosToProcess: Portfolio[]
    ...
    if (portfolioId) {
      ...
      portfoliosToProcess = [portfolio]
    } else {
      // Get portfolios that need snapshots based on configuration
      ...
      if (portfolioIds.length === 0) {
        return new Response(JSON.stringify({
          ...results,
          message: 'No portfolios need snapshots at this time'
        }), {
      ...
      const { data: fullPortfolios, error: portfoliosError } = await supabase
        .from('portfolios')
        .select('*')
        .in('id', portfolioIds)
      
      if (portfoliosError) {
        throw new Error(`Failed to get portfolio details: ${portfoliosError.message}`)
      }
      
      portfoliosToProcess = fullPortfolios || []
    }

    console.log(`Processing ${portfoliosToProcess.length} portfolios`)
    ...
    for (const portfolio of portfoliosToProcess) {
      try {
        ...
        // Update last run timestamp if this wasn't a forced calculation
        if (!forceRecalculation) {
          ...
          // Update each configuration's last run timestamp
          for (const config of configs || []) {
            await supabase.rpc('update_snapshot_last_run', {
              p_portfolio_id: portfolio.id,
              p_interval_hours: config.interval_hours
            })
          }
        }
    ...
    console.log('Portfolio snapshot calculation completed', results)
